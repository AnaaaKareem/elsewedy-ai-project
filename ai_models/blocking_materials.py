...
class SentinelLumpyModel:
    ...
    def save_weights(self, filename):
        ...
        if HAS_NUMPY:
            np.save(f"weights/{filename}.npy", [self.alpha, self.demand_level, self.period_interval])
        else:
            with open(f"weights/{filename}.npy", "w") as f:
                f.write(f"mock_croston:{self.demand_level}:{self.period_interval}")
        print(f"💾 Saved Croston params: weights/{filename}.npy")

    def load_weights(self, filename):
        """
        Load model parameters from disk.
        """
        path = f"weights/{filename}.npy"
